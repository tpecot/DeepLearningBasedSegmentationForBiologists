...
class ProposalLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        deltas = mrcnn_utils.batch_slice([deltas, ix], lambda x, y: tf.gather(x, y),
                                   self.config.IMAGES_PER_GPU)
        anchors = mrcnn_utils.batch_slice(ix, lambda x: tf.gather(anchors, x),
                                    self.config.IMAGES_PER_GPU,
                                    names=["pre_nms_anchors"])

        # Apply deltas to anchors to get refined anchors.
        # [batch, N, (y1, x1, y2, x2)]
        boxes = mrcnn_utils.batch_slice([anchors, deltas],
        ...
        def nms(normalized_boxes, scores):
            ...
            return proposals
        proposals = mrcnn_utils.batch_slice([normalized_boxes, scores], nms,
                                      self.config.IMAGES_PER_GPU)
        return proposals
...
def load_image_gt(dataset, config, image_id, augment=False,
    ...
    shape = image.shape
    image, window, scale, padding = mrcnn_utils.resize_image(
        image,
        min_dim=config.IMAGE_MIN_DIM,
        max_dim=config.IMAGE_MAX_DIM,
        padding=config.IMAGE_PADDING)
    mask = mrcnn_utils.resize_mask(mask, scale, padding)
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        # Generate Anchors
        self.anchors = mrcnn_utils.generate_pyramid_anchors(config.RPN_ANCHOR_SCALES,
                                                      config.RPN_ANCHOR_RATIOS,
                                                      config.BACKBONE_SHAPES,
                                                      config.BACKBONE_STRIDES,
                                                      config.RPN_ANCHOR_STRIDE)
    ...
    def mold_inputs(self, images):
        ...
        for image in images:
            ...
            molded_image, window, scale, padding = mrcnn_utils.resize_image(
                image,
    ...
    def unmold_detections(self, detections, mrcnn_mask, image_shape, window):
        ...
        for i in range(N):
            ...
            full_mask = mrcnn_utils.unmold_mask(masks[i], boxes[i], image_shape)
